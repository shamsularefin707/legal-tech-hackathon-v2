...
import { Phone, Moon, Sun, RotateCcw, Menu, Shield, UserCheck } from 'lucide-react';
    ...
    <header className="sticky top-0 z-30 shadow-xs border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#15202E]">
      ...
      <div className="bg-[#004D3A] text-white border-b border-[#006A4E] text-[11px] sm:text-xs">
        <div className="max-w-[1600px] mx-auto px-3 sm:px-6 py-1.5 flex flex-wrap items-center justify-between gap-2">
          {/* Official Portal Disclaimer & Republic Title */}
          <div className="flex items-center gap-2 font-medium">
            ...
            <span className="font-semibold tracking-wide">গণপ্রজাতন্ত্রী বাংলাদেশ সরকার</span>
            <span className="text-emerald-300/60 hidden md:inline">|</span>
            <span className="text-emerald-100 hidden md:inline">আইন ও বিচার বিভাগ • আইন, বিচার ও সংসদ বিষয়ক মন্ত্রণালয়</span>
          </div>

          {/* Right Utilities: Helpline + System Demo Date + Theme Toggle */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-600 dark:text-slate-400 hidden sm:block">
                জেলা লিগ্যাল এইড অফিস • ৮টি ডিজিটাল পাইলট জেলা সমন্বয় প্ল্যাটফর্ম
              </p>
            </div>
          </div>
        </div>

        {/* Right: Role Switcher + Reset Demo Data */}
        <div className="flex items-center gap-2.5">
          {/* Active Role Selector (Critical for Demoing Role Separation) */}
            ...
            <div className="flex flex-col text-left">
              ...
              >
                {Object.values(ROLE_CONFIGS).map((rc) => (
                  <option key={rc.role} value={rc.role} className="dark:bg-[#15202E] text-slate-900 dark:text-white">
                    {rc.nameBn}
                  </option>
                ))}
              </select>
          ...
          <button
            onClick={onResetDemo}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
            title="ডেমো ডেটাসেট প্রারম্ভিক অবস্থায় রিসেট করুন"
          >
